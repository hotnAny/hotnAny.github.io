...
function renderVoxels(vxg, dim, hideInside) {
	...
	for (var i = 0; i < nz; i++) {
		gVoxelTable[i] = gVoxelTable[i] == undefined ? [] : gVoxelTable[i];
		// var slice = [];
		// EXP: only showing one layer
		for (var j = 0; j < 1; j++) {
			gVoxelTable[i][j] = gVoxelTable[i][j] == undefined ? [] : gVoxelTable[i][j];
			// var row = [];
			for (var k = 0; k < nx; k++) {
				// row[k] = undefined;
				if (vxg[i][j][k] == 1 && gVoxelTable[i][j][k] == undefined) {
					if (hideInside != true || onSurface(i, j, k)) {
						...
						gVoxels.push(voxel);
						gVoxelTable[i][j][k] = voxel;
					}
				} else if (vxg[i][j][k] != 1 && gVoxelTable[i][j][k] != undefined) {
					var voxel = gVoxelTable[i][j][k];
					scene.remove(voxel);
					XAC.removeFromArray(gVoxels, voxel);
					gVoxelTable[i][j][k] = undefined;
				}
			} // x
			// slice.push(row);
		} // y
...
function updateVoxels(vxg, dim, axis, node) {
	...
	var addSphericalVoxels = function(vxg, index, radius) {
		var zmin = XAC.float2int(index[2] - radius),
			zmax = XAC.float2int(index[2] + radius),
			ymin = XAC.float2int(index[1] - radius),
			ymax = XAC.float2int(index[1] + radius),
			xmin = XAC.float2int(index[0] - radius),
			xmax = XAC.float2int(index[0] + radius);
	...
	if (node == undefined) {
		...
		for (var i = axis.nodesInfo.length - 1; i >= 0; i--) {
			...
			vxg[index[2]][index[1]][index[0]] = gMedialAxis.NODE;
			if (radius != undefined) {
		...
		for (var i = axis.edgesInfo.length - 1; i >= 0; i--) {
			...
			for (var j = pts.length - 1; j >= 0; j--) {
				...
				addSphericalVoxels(vxg, pts[j].index, thickness[k]);
			}
		}
