...
in order to find such proteins, scientists have to consider multiple criteria: whether a target protein ever physically interacts with Tau, whether it can have the intended therapeutic impact on Tau, and whether a drug can dock on it to exert its effect.
during our formative studies, we found that conventionally, these different criteria are analyzed separately. scientists have to run back and forth between multiple activities using various tools. sometimes it's running a 3D simulation to consider docking criterion and sometimes it involves time-consuming literature search to find out the kinds of therapeutic impact between proteins.
<!-- while updating/maintaining a list of promising target proteins that aim to meet all criteria as well as possible. -->
there is an opportunity to design a tool that optimize this workflow.
our solution is computing all these criteria in one place and using protein graph as a medium to visualize all three criteria, such as the thickness of edges indicate the strength of physical interaction between a pair of proteins and the colors of the nodes indicate the docking potential given a drug.

this visualization approach is really no ground breaking. it might feel familiar, like some early work on tool support for exploring a large network of research papers or social media accounts. 
indeed, target id belongs to the same class of problems but it has never been viewed in that lens; once you see it this way, the solution is quite straightforward.
further, we also made use of the advances in language models to help us compute the therapeutic criterion, which used to require extensive literature search and reading. now, given a path of proteins that connects a potential target to Tau, we can break it into pairs, use LM to search literature and distill therapuetic insights about them, then also LM to synthesize all these pairwise findings to give us a more conclusive verdict on therapeutic impact.
thanks to LM, we can conveniently automate and integrate laborious literature search as a component of our tool, while keeping it grounded in actual evidence (i.e. research papers) when presenting and explaining the computing results to the scientists.

Summary. (how we tackle the augmentation problem of target ID)
