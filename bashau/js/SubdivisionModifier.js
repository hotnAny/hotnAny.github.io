...
THREE.SubdivisionModifier.prototype.smooth = function ( oldGeometry ) {
	...
	function v( x, y, z ) {
		newVertices.push( new THREE.Vector3( new THREE.Vector3( x, y, z ) ) );
	}
	...
	for (i=0, il = originalFaces.length; i<il ;i++) {
		...
		newPoints.push( new THREE.Vector3(face.centroid) );
	...
	for (i in vfMap) {
		...
		newPoints.push( new THREE.Vector3(avg) );
	...
	for (i=0, il = originalPoints.length; i<il; i++) {
		...
		*/
		...
		for (j=0; j<n;j++) {
			...
			R.add(midPt);
			// R.add(originalPoints[edge[0]].position);
			// R.add(originalPoints[edge[1]].position);
		}
