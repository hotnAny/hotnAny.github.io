 ...
 ( function ( THREE ) { "use strict";
	...
	THREE.Octree.prototype = {
		...
		add: function ( object, options ) {
			...
			// console.log("# of nodes: " + this.root.getNodeCountEnd());
		...
		search: function ( position, radius, organizeByObject, direction ) {
			...
			console.log("searching through " + this.root.getNodeCountEnd() + " nodes ...");
	...
	THREE.OctreeNode = function ( parameters ) {
		...
		if ( this.tree.scene ) {
			...
			this.tree.boxes.add(this.visual);
	...
	THREE.OctreeNode.prototype = {
		...
		addObject: function ( object ) {
			...
			if ( indexOctant > -1 && this.nodesIndices.length > 0 ) {
				...
				// xac::dev
				var onePerNode = false;

				if(onePerNode) {
					node = this;
					do {
						indexOctant = node.getOctantIndex( object );
						node = node.branch( indexOctant );
					} while(node.objects.length > 1);
				} else {
					node = this.branch( indexOctant );
				}

				node.addObject( object );
				// console.log("# of objects in this node: " + node.objects.length);
		...
		checkGrow: function () {
			...
			if ( this.objects.length > this.tree.objectsThreshold && this.tree.objectsThreshold > 0 ) {
				// console.log("object threshold: " + this.tree.objectsThreshold);
				this.grow();
		...
		search: function ( position, radius, objects, direction, directionPct ) {
			...
			if ( intersects === true) {
				...
				// 	addABox(this.left, this.right, this.top, this.bottom, this.front, this.back);
				// 	controlPanel.log("intersecting a box that has " + this.objects.length + " objects and " + this.nodesIndices.length + " nodes");
				// }
				...
				// console.log("intersecting with a box that contains " + objects.length + " objects");
				if(this.objects.length == 1) {
					objects = objects.concat( this.objects );
					addABox(this.left, this.right, this.top, this.bottom, this.front, this.back, false);
		...
		intersectSphere: function ( position, radius ) {
			...
			// if(distance >= 0) {
			// 	addABox(this.left, this.right, this.top, this.bottom, this.front, this.back);
			// 	// controlPanel.log(this.left + ", " + this.right + ", " + this.top + ", " + this.bottom + ", " + this.front + ", " + this.back);
			// }
		...
		getNodeCountRecursive: function () {
			...
			var i, l,
				count = this.tree.root.nodesIndices.length;
