...
//
XDOLLAR.recognize = function (coords) {
    ...
    for (let i = 0, sign = Math.NaN; i < coords.length - step; i++) {
        let vector = XDOLLAR.getVector(coords[i + step], coords[i]);
...
//
XDOLLAR.getVector = function (c1, c2) {
    return {
        x: c2.x - c1.x,
        y: c2.y - c1.y
    };
