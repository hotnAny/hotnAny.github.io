window.Physijs = (function() {
	...
	addObjectChildren = function( parent, object ) {
		...
		for ( i = 0; i < object.children.length; i++ ) {
			if ( object.children[i]._physijs ) {
				...
				// console.log(object.children[i].matrixWorld);
	...
	Physijs.Scene.prototype.remove = function( object ) {
		...
		if ( object && object.material && object.material._physijs && this._materials_ref_counts.hasOwnProperty( object.material._physijs.id ) ) {
			this._materials_ref_counts[object.material._physijs.id]--;
	...
	/* ----  ----  ----  ----  ----  ----  ----  ----  ----  ----  ----  ----  ---- */
	/* simulate from buffered information */
	Physijs.Scene.prototype.simulate = function( timeStep, maxSubSteps) {
		var object_id, object, update;
	...
	Physijs.Scene.prototype.pushBuffered = function() {
		...
		for ( object_id in this._objects ) {
			...
			if(updatesBuffered[object_id] == undefined) {
				updatesBuffered[object_id] = new Array();
				// console.log("created buffer #" + object_id + " - " + obj._physijs.id);
			}

			if ( obj.__dirtyPosition || obj.__dirtyRotation ) {
				if(updatesBuffered[object_id] instanceof Array) {
					var update;

					update = { id: obj._physijs.id };

					if ( obj.__dirtyPosition ) {
						update.pos = { x: obj.position.x, y: obj.position.y, z: obj.position.z };
						obj.__dirtyPosition = false;
					}

					if ( obj.__dirtyRotation ) {
						update.quat = { x: obj.quaternion.x, y: obj.quaternion.y, z: obj.quaternion.z, w: obj.quaternion.w };
						obj.__dirtyRotation = false;
					}

					updatesBuffered[object_id].push(update);
					console.log("# of buffered points: " + updatesBuffered[object_id].length);
				}
			}
		}
	}
